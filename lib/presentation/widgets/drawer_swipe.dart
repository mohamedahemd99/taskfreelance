...
class SwipeDrawerState extends State<SwipeDrawer> with SingleTickerProviderStateMixin {
          ...
          return Stack(
            ...
            children: [
              ...
              if (widget.hasClone)
                Opacity(
                  opacity: .3,
                  child: buildAnimationBody(scaleSmall, size, widget.bodySize + widget.bodyBackgroundPeekSize, reverse, isClone: true),
                ),
              GestureDetector(
                  onHorizontalDragStart: (details) {}, behavior: HitTestBehavior.translucent, child: buildAnimationBody(scale, size, widget.bodySize, reverse)),
              Transform.translate(
  ...
  Container buildDrawer(Size size) {
    return Container(width: size.width - (widget.bodySize + widget.bodyBackgroundPeekSize), child: widget.drawer);
  }

  Transform buildAnimationBody(double scale, Size size, double move, double revers, {bool isClone = false}) {
    return Transform(
        ...
        child: InkWell(
          onTap: () {
            if (isOpened()) openOrClose();
          },
          child: ClipRRect(
            borderRadius: BorderRadius.circular(isOpened() ? widget.radius : 0),
            child: Container(color: Colors.white, child: isClone ? Container() : widget.child),
          ),
        ));
  }
